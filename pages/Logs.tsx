...
import { getLogColor, generatePageSummary } from '../utils/formatters';
...
export const Logs: React.FC = () => {
    ...
    useEffect(() => {
        const handleToolAction = (e: any) => {
            ...
            if (action === 'COPY') {
                const text = generatePageSummary('logs', dataToExport);
                navigator.clipboard.writeText(text).then(() => alert("已复制日志到剪贴板"));
            } else if (action === 'EXCEL') {
                if (!(window as any).XLSX) return alert("Excel 组件未加载");
                
                const excelRows = dataToExport.map(l => ({
                    时间: new Date(l.timestamp).toLocaleString(),
                    操作人: l.operator || '未知',
                    类型: l.type,
                    商品: l.product?.name || '未知',
                    变动数量: l.type === 'OUT' || l.type === 'DELETE' ? `-${l.quantity}` : `+${l.quantity}`,
                    备注: l.note || ''
                }));
                
                const ws = (window as any).XLSX.utils.json_to_sheet(excelRows);
                const wb = (window as any).XLSX.utils.book_new();
                (window as any).XLSX.utils.book_append_sheet(wb, ws, "Logs");
                (window as any).XLSX.writeFile(wb, `Logs_${new Date().toISOString().slice(0,10)}.xlsx`);
            }
        };
